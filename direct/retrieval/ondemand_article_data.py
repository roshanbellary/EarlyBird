...
class OndemandPull():
    ...
    async def continue_fetch(self, batch_size: int = 3):
        ...
        print(f"All finished articles: {len(finished_urls)}")

        # filter out finished articles

        len_total = len(await self.get_article_urls())
        
        unfinished_urls: Tuple[str, str] = [url for url in await self.get_article_urls() if url[0] not in finished_urls]

        # get skipped articles
        skipped_urls = await self.get_skipped()

        unfinished_urls = [url for url in unfinished_urls if url[0] not in skipped_urls]


        print(f"All unfinished articles: {len(unfinished_urls)}")
